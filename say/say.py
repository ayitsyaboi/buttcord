...
class Say(BaseCog):
    """
    Speak as if you were the bot. Supports file attachments. 
    """
    ...
    async def _say(
        ...
        If no channel is specified the message will be sent in the current channel.
        You can attach files to the message.

        Example usage :
        - `-say #general hello there`
        - `-say I have a file` (a file is attached to the command message)
        """

        files = await Tunnel.files_from_attatch(ctx.message)
        await self.say(ctx, channel, text, files)

    #removed due to lack of usefulness in the server
    """
    ...
    async def _sayautodelete(
        ...
        *,
        ...
        #Example: `-sayad 10 this message will be removed after 10 seconds` removes the message after 10 seconds.
    ...
    async def _saydelete(
        ...
        If the message wasn't removed then I don't have the appropriate permissions.
        """
    ...
    #removed due to abuse potential
    """
